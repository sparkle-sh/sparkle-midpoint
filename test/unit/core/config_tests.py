...
class ConfigTests(unittest.TestCase):
    ...
    def test_read_config_from_invalid_path_expect_throw(self):
        self.assertRaises(error.ConfigError, lambda: config.Config(
            "this_path_is_definitely_invalid"))

    @ddt.data(*CORRUPTED_CONFIGS)
    def test_read_corrupted_config_except_throw(self, cfg):
        self.create_config_file(cfg)
        self.assertRaises(error.ConfigError,
                          lambda: config.Config(TEST_CONFIG))
    ...
    def test_read_valid_config_except_correct_fields(self):
        ...
        self.assertEqual(cfg.connector.host,
                         raw_cfg['services']['connector']['host'])
        self.assertEqual(cfg.connector.port,
                         raw_cfg['services']['connector']['port'])
